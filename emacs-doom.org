...
between all Emacs configs. File templates, especially automatic ones, are handy for multi-faceted
...
Doom has its own code for automatically applying file templates ([[https://docs.doomemacs.org/latest/modules/editor/file-templates/][modules/editor/file-templates]]).
In non-Doom configs, I might settle for manual invocation;
most default file templates are named =__= by convention.
